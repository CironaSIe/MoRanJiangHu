...
const 标准化角色档案 = (raw: any): 小说拆分分段结构['角色档案'][number] => ({
    名称: 读取文本(raw?.名称).trim(),
    身份: 读取文本(raw?.身份).trim(),
    所属势力: 读取文本(raw?.所属势力).trim(),
    初始立场: 读取文本(raw?.初始立场).trim(),
    关系摘要: 标准化文本数组(raw?.关系摘要),
    状态摘要: 标准化文本数组(raw?.状态摘要),
    首次出现: 读取文本(raw?.首次出现).trim(),
    重要性: raw?.重要性 === '核心' || raw?.重要性 === '重要' ? raw.重要性 : '一般'
});

const 标准化势力档案 = (raw: any): 小说拆分分段结构['势力档案'][number] => ({
    名称: 读取文本(raw?.名称).trim(),
    类型: 读取文本(raw?.类型).trim(),
    地盘: 读取文本(raw?.地盘).trim(),
    代表人物: 标准化文本数组(raw?.代表人物),
    立场目标: 读取文本(raw?.立场目标).trim(),
    当前状态: 读取文本(raw?.当前状态).trim(),
    关系摘要: 标准化文本数组(raw?.关系摘要),
    首次出现: 读取文本(raw?.首次出现).trim()
});

const 标准化地图地点档案 = (raw: any): 小说拆分分段结构['地图地点档案'][number] => ({
    名称: 读取文本(raw?.名称).trim(),
    层级: raw?.层级 === '具体地点'
        ? '区地点'
        : raw?.层级 === '寰宇' || raw?.层级 === '大地点' || raw?.层级 === '中地点' || raw?.层级 === '小地点' || raw?.层级 === '区地点' || raw?.层级 === '子地点'
            ? raw.层级
            : '未知',
    上级地点: 读取文本(raw?.上级地点).trim(),
    所属势力: 读取文本(raw?.所属势力).trim(),
    地貌功能: 读取文本(raw?.地貌功能).trim(),
    关键设施: 标准化文本数组(raw?.关键设施),
    首次出现: 读取文本(raw?.首次出现).trim()
});

const 标准化物品档案 = (raw: any): 小说拆分分段结构['物品档案'][number] => ({
    名称: 读取文本(raw?.名称).trim(),
    类型: 读取文本(raw?.类型).trim(),
    用途: 读取文本(raw?.用途).trim(),
    所属人物: 读取文本(raw?.所属人物).trim(),
    所属势力: 读取文本(raw?.所属势力).trim(),
    首次出现: 读取文本(raw?.首次出现).trim()
});
...
const 标准化分段 = (raw: any, 数据集ID: string): 小说拆分分段结构 => {
    ...
    return {
        ...
        角色档案: Array.isArray(raw?.角色档案) ? raw.角色档案.map((item: unknown) => 标准化角色档案(item)).filter((item) => item.名称) : [],
        势力档案: Array.isArray(raw?.势力档案) ? raw.势力档案.map((item: unknown) => 标准化势力档案(item)).filter((item) => item.名称) : [],
        地图地点档案: Array.isArray(raw?.地图地点档案) ? raw.地图地点档案.map((item: unknown) => 标准化地图地点档案(item)).filter((item) => item.名称) : [],
        物品档案: Array.isArray(raw?.物品档案) ? raw.物品档案.map((item: unknown) => 标准化物品档案(item)).filter((item) => item.名称) : [],
        世界观规则: 标准化文本数组(raw?.世界观规则),
        世界边界规则: 标准化文本数组(raw?.世界边界规则),
        人物关系: 标准化文本数组(raw?.人物关系),
        势力关系: 标准化文本数组(raw?.势力关系),
        伏笔线索: 标准化文本数组(raw?.伏笔线索),
        回收点: 标准化文本数组(raw?.回收点),
        章节节奏: 标准化文本数组(raw?.章节节奏),
        时间线,
        时间线起点,
        时间线终点,
...
export const 读取小说拆分数据集列表 = async (): Promise<小说拆分数据集结构[]> => {
    ...
    return raw.map((item) => 规范化小说拆分数据集(item));
};
...
export const 保存小说拆分数据集列表 = async (items: 小说拆分数据集结构[]): Promise<void> => {
    ...
    await dbService.保存设置(设置键.小说分解数据集, normalizedList.map((item) => 深拷贝(item)));
};
...
export const 创建空小说拆分数据集 = (params?: Partial<小说拆分数据集结构>): 小说拆分数据集结构 => {
    ...
    return 规范化小说拆分数据集({
        ...
        角色档案: params?.角色档案 || [],
        势力档案: params?.势力档案 || [],
        地图地点档案: params?.地图地点档案 || [],
        物品档案: params?.物品档案 || [],
        世界观规则: params?.世界观规则 || [],
        世界边界规则: params?.世界边界规则 || [],
        人物关系: params?.人物关系 || [],
        势力关系: params?.势力关系 || [],
        伏笔线索: params?.伏笔线索 || [],
        回收点: params?.回收点 || [],
...
export const 写入小说拆分数据集 = async (dataset: 小说拆分数据集结构): Promise<小说拆分数据集结构[]> => {
    ...
    const merged = hasExisting
        ...
        : [normalized, ...list];
    const next = merged.map((item) => ({ ...item, 激活注入: false }));
...
export const 删除小说拆分数据集 = async (datasetId: string): Promise<{
    ...
    const nextDatasets = datasets.filter((item) => item.id !== datasetId).map((item) => ({ ...item, 激活注入: false }));
