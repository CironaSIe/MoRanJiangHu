...
const 获取显式拍卖货币系统 = (options?: 货币格式化选项) => (
...
export const 格式化拍卖货币 = (value: number, currency: 拍卖货币 = '铜钱', options?: 货币格式化选项) => {
    ...
    const currencySystem = 获取显式拍卖货币系统(options);
    if (currencySystem) {
...
const 规范化拍卖行状态 = (parsed: Partial<拍卖行状态> | null | undefined, mode?: 拍卖题材模式): 拍卖行状态 => 清理并补货({
...
}, { 允许系统补货: false, 题材模式: mode }); // 读取时不自动补货，等待剧情触发
...
export const 读取拍卖行状态 = (scope?: string, options?: { 题材模式?: 拍卖题材模式 }): 拍卖行状态 => {
    if (typeof window === 'undefined') return 清理并补货(创建空拍卖行状态(), { 允许系统补货: false, 题材模式: options?.题材模式 });
    try {
        const scopedKey = 获取拍卖行存储键(scope);
        const raw = window.localStorage.getItem(scopedKey);
        if (!raw) return 清理并补货(创建空拍卖行状态(), { 允许系统补货: false, 题材模式: options?.题材模式 });
        const parsed = JSON.parse(raw) as Partial<拍卖行状态>;
        return 规范化拍卖行状态(parsed, options?.题材模式);
    } catch {
        return 清理并补货(创建空拍卖行状态(), { 允许系统补货: false, 题材模式: options?.题材模式 });
    }
};
...
const 清理题材行情列表 = (previous: 拍卖行情[] = [], mode?: 拍卖题材模式): 拍卖行情[] => {
    const now = Date.now();
    const templateKeys = new Set(获取题材行情模板(mode).map((item) => `${item.标题}|${item.热点标签}`));
    return previous.filter((item) => {
        if (!item || item.过期时间 <= now) return false;
        if (行情是否不符合题材(item, mode)) return false;
        return templateKeys.has(`${item.标题}|${item.热点标签}`);
    });
};

export const 生成行情列表 = (force = false, previous: 拍卖行情[] = [], previousTime = 0, mode?: 拍卖题材模式): { 行情列表: 拍卖行情[]; 最近行情时间: number } => {
    const now = Date.now();
    const activePrevious = 清理题材行情列表(previous, mode);
...
const 题材默认拍卖物品池: Record<string, 默认拍卖物品模板[]> = {
    wuxia: [
        ...
        { 名称: '软皮护腕', 类型: '防具', 品质: '凡品', 描述: '轻便护腕，适合赶路和近身缠斗。', 价值: 140, 标签: ['护具'], 装备位置: '手部', 物理防御: 3, 内功防御: 1 },
    ...
    modern: [
        { 名称: '急救包', 类型: '消耗品', 品质: '凡品', 描述: '包含绷带、消毒片和基础止血用品，可恢复少量气血。', 价值: 90, 标签: ['急救'], 使用效果: [{ 目标属性: '全身血量', 数值: 35, 依据: '绷带包扎止血' }] },
        { 名称: '多功能工具钳', 类型: '杂物', 品质: '良品', 描述: '可用于开箱、维修和现场处理小故障。', 价值: 160, 标签: ['工具'] },
        { 名称: '防割手套', 类型: '防具', 品质: '凡品', 描述: '适合外勤、仓储或危险现场使用，提供基础手部防护。', 价值: 130, 标签: ['防护'], 装备位置: '手部', 物理防御: 4, 内功防御: 0 },
    ...
    urban_xianxia: [
        ...
        { 名称: '便携医疗包', 类型: '消耗品', 品质: '凡品', 描述: '现实急救用品，适合处理轻中度外伤，可恢复少量气血。', 价值: 110, 标签: ['急救'], 使用效果: [{ 目标属性: '全身血量', 数值: 40, 依据: '医疗包处理外伤' }] },
        { 名称: '低阶灵晶碎片', 类型: '材料', 品质: '上品', 描述: '异常复苏后流通的小块能量结晶。', 价值: 680, 标签: ['灵晶'] }
    ],
    apocalypse: [
        { 名称: '净水片', 类型: '消耗品', 品质: '凡品', 描述: '能临时处理少量可疑水源。', 价值: 70, 标签: ['生存物资'] },
        { 名称: '简易防刺背心', 类型: '防具', 品质: '良品', 描述: '营地自制护具，能挡住抓咬和碎片划伤。', 价值: 260, 标签: ['防护'], 装备位置: '胸部', 物理防御: 12, 内功防御: 2 },
        { 名称: '手摇电筒', 类型: '杂物', 品质: '凡品', 描述: '不依赖电池的照明工具，亮度有限但可靠。', 价值: 120, 标签: ['照明'] },
        { 名称: '罐头补给包', 类型: '消耗品', 品质: '凡品', 描述: '压缩打包的基础食物，够一人撑过短途行动。', 价值: 100, 标签: ['食物'] }
    ],
    infinite: [
        { 名称: '止血喷雾', 类型: '消耗品', 品质: '凡品', 描述: '主神商城常见基础补给，可快速处理开放性伤口，恢复少量气血。', 价值: 80, 标签: ['主神商城', '基础补给'], 使用效果: [{ 目标属性: '全身血量', 数值: 30, 依据: '止血喷雾处理伤口' }] },
        { 名称: '九毫米手枪', 类型: '武器', 品质: '良品', 描述: '适合新人任务世界防身的基础火器。', 价值: 350, 标签: ['主神商城', '武器'] },
        { 名称: '轻型防护背心', 类型: '防具', 品质: '良品', 描述: '能抵挡部分撕咬、钝击和低速弹片，提供躯干防护。', 价值: 280, 标签: ['主神商城', '防具'], 装备位置: '胸部', 物理防御: 15, 内功防御: 3 },
        { 名称: '基础精神稳定剂', 类型: '消耗品', 品质: '上品', 描述: '短时间缓解恐怖片任务中的惊惧和精神污染。', 价值: 520, 标签: ['主神商城', '精神防护'] }
    ],
    western_fantasy: [
        { 名称: '小瓶治疗药水', 类型: '消耗品', 品质: '凡品', 描述: '冒险者常备的基础治疗药水，可恢复少量气血。', 价值: 100, 标签: ['药水'], 使用效果: [{ 目标属性: '全身血量', 数值: 40, 依据: '治疗药水恢复气血' }] },
        { 名称: '铁制短剑', 类型: '武器', 品质: '良品', 描述: '佣兵和新手冒险者常用的短剑。', 价值: 260, 标签: ['武器'] },
        { 名称: '皮革胸甲', 类型: '防具', 品质: '凡品', 描述: '轻便耐磨，适合探索和巡逻，提供基础躯干防护。', 价值: 220, 标签: ['护甲'], 装备位置: '胸部', 物理防御: 8, 内功防御: 2 },
...
export const 投放事件拍卖品并保存 = (params: 拍卖行事件投放参数, scope?: string): 拍卖行状态 => {
    const next = 投放事件拍卖品(读取拍卖行状态(scope, { 题材模式: params.题材模式 }), params);
    保存拍卖行状态(next, scope);
