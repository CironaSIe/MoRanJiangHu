...
const 以总铜钱更新角色金钱 = (character: 角色数据结构, nextTotalCopper: number, options?: 货币格式化选项): 角色数据结构 => ({
    ...character,
    金钱: 底层总值转角色金钱(nextTotalCopper, options?.runtimeProfile, 读取货币模式(options)),
});
...
export const 自动扣除铜钱 = (character: 角色数据结构, copperCost: number, options?: 货币格式化选项) => {
    ...
    return {
        ok: true as const,
        nextCharacter: 以总铜钱更新角色金钱(character, owned - cost, options),
        paidCopper: cost,
...
export const 自动增加铜钱 = (character: 角色数据结构, copperIncome: number, options?: 货币格式化选项): 角色数据结构 => (
    以总铜钱更新角色金钱(character, 计算金钱铜钱总值(character?.金钱) + Math.max(0, Math.floor(读数(copperIncome))), options)
);
...
export const 购买拍卖品 = (character: 角色数据结构, auction: 拍卖品记录, options?: 货币格式化选项) => {
    ...
    const copperCost = price * 获取货币倍率(currency, options);
...
export const 结算玩家寄售 = (
    ...
    const nextAuctions = (state.拍卖品列表 || []).map((entry) => {
        ...
        const copper = Math.max(1, 读数(entry.一口价 || entry.当前价格)) * 获取货币倍率(entry.标价货币 || '铜钱', options);
        totalCopper += copper;
        const settled: 拍卖品记录 = {
            ...entry,
            状态: '已成交',
    ...
    const nextCharacter = 自动增加铜钱(character, totalCopper, options);
...
export const 执行货币换兑 = (character: 角色数据结构, from: 拍卖货币, to: 拍卖货币, amount: number, options?: 货币格式化选项) => {
    ...
    const feeRate = 0.03;
    const copperValue = count * 获取货币倍率(from, options);
    const targetValue = Math.floor((copperValue / 获取货币倍率(to, options)) * (1 - feeRate));
...
export const 执行自动货币整理 = (character: 角色数据结构, options?: 货币格式化选项) => {
    ...
    return {
        ok: true as const,
        nextCharacter: 以总铜钱更新角色金钱(character, total, options),
        totalCopper: total,
