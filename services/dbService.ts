...
import { 创建图片资源引用, 解析图片资源引用ID, 是否图片资源引用, 注册图片资源缓存, 批量注册图片资源缓存, 清空图片资源缓存, 注册远程图片兜底引用, 读取远程图片兜底资源ID集合 } from '../utils/imageAssets';
...
const 读取已引用图片资源ID集合 = async (): Promise<Set<string>> => {
    ...
    读取远程图片兜底资源ID集合().forEach((id) => refs.add(id));
    return refs;
};
...
export const 读取本地图片资源统计 = async (): Promise<本地图片资源统计> => {
    try {
        ...
        const remoteEntries = referencedEntries.filter((item) => 是远程地址(item.dataUrl || ''));
        return {
            ...
            remoteImageAssets: remoteEntries.length,
            migrationStatus: 获取本地图片图床迁移状态()
        };
    } catch (error) {
...
export const 自动迁移本地图片到图床 = async (): Promise<本地图片图床自动迁移结果> => {
    ...
    try {
        ...
        更新本地图片图床迁移状态({
            ...
            retryLater: false,
            lastError: undefined,
            lastMessage: `正在迁移旧存档图片，待处理 ${candidates.length} 张`
        });
        ...
        更新本地图片图床迁移状态({
            ...
            lastMessage: remainingFailedAssets.length > 0
                ? `旧存档图片已迁移 ${replacements.size} 张，${remainingFailedAssets.length} 张稍后自动重试`
                : `旧存档图片迁移完成，已替换 ${replacements.size} 张图床链接`
        });
