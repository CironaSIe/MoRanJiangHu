...
const 查找重复本地图片资源 = (
    ...
    groups.forEach((items) => {
        ...
        const keeper = items.find((item) => directReferencedIds.has(item.id) && !是否远程图片本地备份ID(item.id))
            ...
            || items[0];
        if (!keeper?.id) return;
        const groupRemoteUrl = items.map((item) => remoteByAssetId.get(item.id) || '').find(Boolean) || '';
        if (groupRemoteUrl && directReferencedIds.has(keeper.id)) {
            replacements.set(创建图片资源引用(keeper.id), groupRemoteUrl);
        }
        items.forEach((item) => {
            if (item.id === keeper.id) return;
            if (directReferencedIds.has(item.id)) {
                replacements.set(创建图片资源引用(item.id), groupRemoteUrl || 创建图片资源引用(keeper.id));
            }
...
export const 自动迁移本地图片到图床 = async (): Promise<本地图片图床自动迁移结果> => {
    ...
    try {
        ...
        reusedRemoteUrls.forEach((remoteUrl, assetId) => {
            replacements.set(创建图片资源引用(assetId), remoteUrl);
        });
    ...
    try {
        ...
        if (legacySaves.length <= 0) {
            return 写入旧存档谱系迁移状态({
                ...
                legacySaves: 0,
                convertedSaves: 0,
                skippedSaves: sorted.length,
                failedSaves: 0,
                completedAt: new Date().toISOString(),
                lastMessage: '没有需要转换的旧存档。'
            });
        }
