...
const 读取存档设置图片引用快照 = async (): Promise<{ referencedIds: Set<string>; remoteUrls: Set<string> }> => {
    const db = await 初始化数据库();
    ...
    return { referencedIds, remoteUrls };
};
...
export const 读取本地图片图床迁移资源状态列表 = async (): Promise<本地图片图床迁移资源状态[]> => {
    ...
    const localCandidates = assetEntries.filter((item) => (
        referenceSnapshot.referencedIds.has(item.id) && typeof item.dataUrl === 'string' && 是DataUrl图片(item.dataUrl)
    ));
    return 构建迁移资源状态列表(localCandidates, referenceSnapshot.remoteUrls, assetMap, 读取远程图片兜底映射());
};
...
export const 自动迁移本地图片到图床 = async (): Promise<本地图片图床自动迁移结果> => {
    ...
    try {
        ...
        if (candidates.length <= 0 && remoteBackupCandidates.length <= 0) {
            更新本地图片图床迁移状态({
                stage: 'completed',
        ...
        更新本地图片图床迁移状态({
            ...
            lastMessage: remainingFailedAssets.length > 0
                ...
                : `旧存档图片处理完成，已替换 ${replacements.size} 张图床链接，补齐本地兜底 ${backedUpAssets} 张`
        });
