...
export const 构建最终图片提示词 = (
    ...
    const hasExplicitSize = Boolean(sizeMatch)
        || (Number.isFinite(pngWidth) && pngWidth > 0 && Number.isFinite(pngHeight) && pngHeight > 0);
    if (!hasExplicitSize && composition === '头像' && Number.isFinite(parsedWidth) && Number.isFinite(parsedHeight) && parsedWidth > 0 && parsedHeight > 0 && parsedWidth !== parsedHeight) {
        size = '1024x1024';
        parsedWidth = 1024;
        parsedHeight = 1024;
    }
    ...
    const 需要默认中国人物 = (
        是否角色构图(composition)
        || (composition === '场景' && options?.场景类型 !== '风景场景')
    )
...
const 解析场景词组响应 = (rawText: string): {
    ...
    const combined = `${判定文本}\n${场景类型文本}\n${判定说明}`;
    const 明确剧照 = /剧照场景|剧照|电影剧照|cinematic still/i.test(combined);
    const 明确不适合快照 = /不适合场景快照|风景场景|风景|景观|山水|landscape/i.test(combined) && !明确剧照;
    const 明确适合快照 = /适合场景快照|场景快照/i.test(`${判定文本}\n${场景类型文本}`) && !/不适合/i.test(`${判定文本}\n${场景类型文本}`);
    const 场景类型: 场景生成类型 = 明确剧照
        ? '剧照场景'
        : 明确适合快照 && !明确不适合快照
        ? '场景快照'
        : '风景场景';
...
export const generateNpcSecretPartImagePrompt = async (
    ...
    const taskPrompt = isNovelAI ? [
        ...
        '',
        '【额外生成要求】',
        '世界观：跟随输入资料、角色身份和附加要求；若附加要求指定现代都市、末日丧尸或灵气复苏，禁止回退到古风、武侠或仙侠服饰与场景。',
        `目标部位：${部位}`,
...
export const generateNpcImagePrompt = async (
    ...
    const taskPrompt = isNovelAI ? [
        ...
        '世界观：跟随 NPC 基础资料、正文语境和附加要求；附加要求指定现代都市、末日丧尸、灵气复苏等现代语境时，禁止默认补古风、汉服、仙侠长袍、山门、丹炉、仙雾或 Ancient Chinese Fantasy 底座。',
        `构图：${构图}`,
        '输出结构：请只输出 <提示词>...</提示词>。',
...
export const generateSceneImagePrompt = async (
    ...
    const 强制构图 = 构图要求 === '纯场景' || 构图要求 === '故事快照' || 构图要求 === '剧照';
    const 场景风格要求 = 风格提示词输入 || '跟随正文、场景资料和附加要求决定';
    const 默认人物族裔要求 = 纯场景模式
        ? ''
        : '人物默认：中国人 / East Asian facial features / black or dark brown hair / dark brown eyes。除非正文、角色资料、附加要求或角色锚点明确说明角色是外国人、白人、金发蓝眼、西方贵族等，否则禁止把人物画成欧美脸、金发蓝眼或北欧特征。';
    const 默认系统提示词 = 强制构图 ? [
        '你是通用叙事场景提示词转换器。',
        '任务：把当前场景整理成可直接生图的高质量英文 tags。',
        `目标画风：${场景风格要求}。除非正文、附加要求或风格提示词明确指定，否则不要擅自锁定二次元、写实、国风等具体风格标签。`,
        '世界观与时代语境必须跟随当前正文、场景资料和附加要求；若附加要求指定现代都市、末日丧尸或灵气复苏，禁止补古风建筑、山门洞府、仙雾宫阙、丹炉药阁或传统武侠/仙侠服饰。',
        默认人物族裔要求,
        纯场景模式
        ...
        构图要求 === '剧照'
            ? '剧照模式时，把人物互动、地点环境、光影层次和镜头语言同时做清楚，画面像一帧电影剧照，不要只剩人物或只剩背景。'
            : 构图要求 === '故事快照'
        ...
        构图要求 === '纯场景'
            ...
            : 构图要求 === '剧照'
                ? '构图要求：剧照，电影剧照式单帧画面，人物、景别、前中后景、动作和光影必须同时成立；避免只有人没景或有景没人。'
        ...
        '你是通用叙事场景提示词判定与转换器。',
        '任务：判断当前正文更适合生成“风景场景”“故事快照”还是“剧照场景”，并整理成可直接生图的场景词组。',
        ...
        '剧照场景优先信号：正文同时具备人物、地点、动作、光影或镜头关系，可做成电影剧照式单帧画面；剧照必须有人物也有环境。',
        ...
        '3) <判定说明>...</判定说明>',
        '4) <场景类型>场景快照、风景场景 或 剧照场景</场景类型>',
        '5) 若为风景场景，输出 <提示词结构><基础>...</基础></提示词结构>；若为场景快照或剧照场景，输出 <提示词结构><基础>...</基础><角色>[1]角色名称|tags\n[2]角色名称|tags</角色></提示词结构>'
    ].filter(Boolean).join('\n');
    const taskPrompt = [
        '【环境层级与具体坐标】',
        ...
        构图要求 === '纯场景'
            ...
            : 构图要求 === '剧照'
                ? '构图要求：剧照，电影剧照式单帧镜头，必须同时写清人物、地点、前中后景、镜头焦段、光影和动作；避免只有人物没景或只有风景没人。'
            : 构图要求 === '故事快照'
