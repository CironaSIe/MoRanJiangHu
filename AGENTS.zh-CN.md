...
当项目新增或修改重要机制时，应在 `docs/` 目录下维护对应的机制分析文档。

**文档清单**（docs/ 目录）：
- `AI集成与提示词系统机制分析.md` — API 调用、提示词构建、DeepSeek/GPT/Gemini 兼容性
- `状态管理与持久化机制分析.md` — 存档、读档、回档、IndexedDB、云同步
- `战斗与地图系统机制分析.md` — 战斗流程、六层地图树、A*寻路、地图更新
- `NPC社交与记忆系统机制分析.md` — NPC结构、四层记忆、生图工作流、位置同步
- `任务与门派系统机制分析.md` — 任务类型、门派贡献、约定系统、拍卖行
- `UI组件与渲染系统机制分析.md` — 布局、主题、移动端适配、模态框系统
- `在线功能与创意工坊机制分析.md` — 在线状态、创意工坊、小说分解、云同步
- `题材模式与配置系统机制分析.md` — 8种题材、OpeningConfig、ModeRuntimeProfile
- `创意工坊字段完整参考.md` — 全部字段的路径、类型、默认值、行为说明
- `修改计划.md` — 整体修改计划和执行记录

**维护原则**：
- 修改机制代码后，应同步更新对应的机制分析文档
- 新增字段或功能时，先更新消费信息的文档，再更新相关机制文档
- 删除过时的文档，避免新旧文档并存造成混淆
- 文档中的文件路径和行号应保持最新（可用 serena 工具验证）

### 代码修改后的检查清单
修改代码后，除了 `npm run build` 编译验证外，还应检查：
1. 是否有对应的机制分析文档需要更新
2. AGENTS.md 中的"Relevant Files"是否需要补充
3. 是否有过期文档需要清理
