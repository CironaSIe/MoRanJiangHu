...
const 读取地图层级类型 = (item: any): 地图层级类型 => {
    ...
    if (raw === '具体地点') return '区地点';
    if ((['寰宇', '大地点', '中地点', '小地点', '区地点', '子地点'] as const).includes(raw as 地图层级类型)) return raw as 地图层级类型;
    ...
    if (/室内|房间|寝居|卧房|内室|建筑物内部|楼层|大厅|平台|走廊|通道|地下室|阁楼/u.test(raw)) return '子地点';
    if (/具体地点|建筑|地标|客栈|楼|阁|堂|殿|院|馆|铺|坊/u.test(raw)) return '区地点';
    return '小地点';
};
...
/**
 * 新版地图层级统一为六层；旧存档中的“具体地点”会在读取时归一为“区地点”。
 */
...
const 是否室内居所位置 = (text: unknown): boolean => 文本包含任一(text, [
    ...
    '内部',
    // 独立的"室"字（书室/暖室 等），配合 "子地点" 层级才生效
    '室',
]);

const 是否室内层级 = (layer?: 地图层级结构 | null): boolean => {
    if (!layer) return false;
    // 新地图六层里，子地点对应建筑内部房间；旧“具体地点”已在读取时归一为区地点。
    // 关键词扩充（庙/寺/观/祠/殿/塔/室/府/馆/内部）以覆盖"荒庙内部"等用户用例。
    if (layer.层级 !== '子地点' && layer.层级 !== '区地点') return false;
    return 是否室内居所位置(`${layer.名称}${layer.描述}${layer.归属?.小地点 || ''}`);
};

const 是否细节卡片层级 = (layer?: 地图层级结构 | null): boolean => (
    Boolean(layer && (layer.层级 === '区地点' || layer.层级 === '子地点'))
);
...
const 是否聚落层级 = (layer: 地图层级结构): boolean => {
    ...
    if (layer.层级 === '区地点' || layer.层级 === '子地点') return false;
...
const 规整道路路径点到层级 = (
    ...
    const width = Math.max(1, Number(layer.网格宽度) || 1);
    const height = Math.max(1, Number(layer.网格高度) || 1);
    const safePadding = Math.max(0, Math.min(padding, width / 2, height / 2));
    const maxX = Math.max(safePadding, width - safePadding);
    const maxY = Math.max(safePadding, height - safePadding);
    return (Array.isArray(rawPoints) ? rawPoints : [])
        .filter((point) => Number.isFinite(point?.x) && Number.isFinite(point?.y))
        .map((point) => ({
            x: 限制数值(point.x, safePadding, maxX),
            y: 限制数值(point.y, safePadding, maxY),
        }));
};
...
const 规划避让建筑正交路径 = (
    ...
    const sourcePoints = (Array.isArray(rawPoints) ? rawPoints : [])
        ...
        .filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y));
    if (sourcePoints.length < 2) return 规整道路路径点到层级(sourcePoints, layer);
    ...
    return 规整道路路径点到层级(简化正交路径(routed), layer);
};
...
const 从旧版字段派生地图空间 = (
    ...
    if (取文本(options?.env?.具体地点)) {
        ensureLayer({
            ...
            描述: `${取文本(options?.env?.具体地点)} 的建筑或地标节点。`,
            锚点坐标: { x: 12, y: 12 },
        });
    }
    ...
    const selectTargetLayerForBuilding = (building: any): string => {
        ...
        const exactSpecific = layers.find((layer) => layer.层级 === '区地点' && 匹配层级与归属(layer, ownership, buildingName));
    ...
    layers.forEach((layer) => {
        ...
        rawBuildings.forEach((name, index) => {
            ...
            const childId = isSpecificName
                ? 通过归属查找层级ID('区地点', childOwnership, name)
                : layer.层级 === '大地点'
    ...
    const findParentLayer = (layer: 地图层级结构): 地图层级结构 | null => {
        ...
        return candidates
            .map((candidate) => {
                ...
                if (candidate.层级 === '小地点') score += layer.层级 === '区地点' ? 18 : 0;
                if (candidate.层级 === '中地点') score += layer.层级 === '小地点' ? 18 : layer.层级 === '区地点' ? 8 : 0;
    ...
    if (envSpecificName && 是否室内居所位置(envSpecificName)) {
        const specificLayer = layers.find((layer) => layer.层级 === '区地点' && 层级名称命中(layer.名称, envSpecificName));
    ...
    const envCurrentLayer = layers.find((layer) => (
        (取文本(options?.env?.具体地点) && layer.层级 === '区地点' && 层级名称命中(layer.名称, options?.env?.具体地点))
...
const 按层级补齐道路 = (
    ...
    layers.forEach((layer) => {
        ...
        roads
            ...
            .forEach((road) => {
                if (是否卷轴路线(road)) {
                    road.路径点 = 规整道路路径点到层级(road.路径点, layer);
                    return;
                }
        ...
        if (是否聚落层级(layer) && layerBuildings.length > 0) {
            ...
            roads
                .filter((road) => road.所在层级ID === layer.ID)
                .forEach((road) => {
                    road.路径点 = 是否卷轴路线(road)
                        ? 规整道路路径点到层级(road.路径点, layer)
                        : 规划避让建筑正交路径(road.路径点, layer, layerBuildings);
                });
        }
...
const 查找当前层级 = (
    ...
    if (matchedBuildings.length > 0) {
        const exactLayer = 查找最佳名称层级(layers, '子地点', env?.具体地点)
            || 查找最佳名称层级(layers, '区地点', env?.具体地点);
        if (exactLayer) {
    ...
    const exactSpecific = 查找最佳名称层级(layers, '子地点', env?.具体地点)
        || 查找最佳名称层级(layers, '区地点', env?.具体地点);
