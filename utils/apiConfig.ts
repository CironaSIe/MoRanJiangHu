...
export const 获取记忆精炼接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    const refineModel = 读取字符串(feature?.记忆精炼使用模型).trim();
    if (!refineModel) return null;
    const refineBaseUrl = 读取字符串(feature?.记忆精炼API地址).trim();
    const refineApiKey = 读取字符串(feature?.记忆精炼API密钥).trim();
    const supplier = refineBaseUrl ? 推断供应商(refineBaseUrl) : current.供应商;

    return {
        ...current,
        供应商: supplier,
        协议覆盖: refineBaseUrl ? 'auto' : current.协议覆盖,
        baseUrl: refineBaseUrl || current.baseUrl,
        apiKey: refineApiKey || current.apiKey,
        model: refineModel
    };
};
...
export const 获取地图生成接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    if (!current) return null;
    const feature = (settings as any)?.功能模型占位;
    const mapModel = 读取字符串(feature?.地图生成使用模型).trim();
    if (!mapModel) return current;
    const mapBaseUrl = 读取字符串(feature?.地图生成API地址).trim();
    const mapApiKey = 读取字符串(feature?.地图生成API密钥).trim();
    const supplier = mapBaseUrl ? 推断供应商(mapBaseUrl) : current.供应商;
    return {
        ...current,
        供应商: supplier,
        协议覆盖: mapBaseUrl ? 'auto' : current.协议覆盖,
        baseUrl: mapBaseUrl || current.baseUrl,
        apiKey: mapApiKey || current.apiKey,
        model: mapModel
    };
};
...
export const 获取地图自动更新接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    const mapModel = 读取字符串(feature?.地图自动更新使用模型).trim();
    if (!mapModel) return null;
    const mapBaseUrl = 读取字符串(feature?.地图自动更新API地址).trim();
    const mapApiKey = 读取字符串(feature?.地图自动更新API密钥).trim();
    const supplier = mapBaseUrl ? 推断供应商(mapBaseUrl) : current.供应商;
    return {
        ...current,
        供应商: supplier,
...
export const 刷新已发现ComfyUI后端缓存 = async (registryUrl?: string, connectToken?: string): Promise<已发现ComfyUI后端缓存项[]> => {
    try {
        const { fetchDiscoveredImageBackends, sortDiscoveredImageBackendsByPreference, isImageBackendRecentlyUnavailable } = await import('../services/ai/imageBackendRegistry');
...
export const 标记ComfyUI后端近期不可用 = async (baseUrl: string): Promise<void> => {
    ...
    if (!url) return;
    try {
        const { recordImageBackendConnectionFailure } = await import('../services/ai/imageBackendRegistry');
