...
const SocialModal: React.FC<Props> = ({
    ...
    const PrivateTag: React.FC<{ label: string; value?: string; color?: string }> = ({ label, value, color = "text-pink-300" }) => (
        <div className="social-private-tag flex flex-col bg-black/40 border border-gray-800 p-2 rounded relative group hover:border-pink-500/50 transition-colors">
            <span className="social-private-label text-[9px] text-gray-500 uppercase tracking-widest mb-1">{label}</span>
            <span className={`social-private-value font-serif text-sm ${color} drop-shadow-sm`}>{value || "???"}</span>
        </div>
    );
    const 首次亲密档案框: React.FC = () => {
        ...
        return (
            <div className="social-intimacy-archive mb-5 rounded-lg border border-pink-900/35 bg-black/45 p-3">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                    <span className="text-[10px] font-bold tracking-[0.22em] text-pink-300">NSFW经历档案</span>
                    {当前失贞档案 && (
                        <span className={`rounded border px-2 py-0.5 text-[10px] ${当前失贞档案.是否失贞 ? 'border-wuxia-gold/40 text-wuxia-gold bg-wuxia-gold/10' : 'border-pink-700/40 text-pink-300 bg-pink-950/20'}`}>
                            {当前失贞档案.是否失贞 ? '已失贞' : '未失贞'}
                        </span>
                    )}
                </div>
    ...
    const RelationTag: React.FC<{ label: string; value?: string; accent?: string }> = ({ label, value, accent = "text-cyan-300" }) => (
        <div className="social-relation-tag bg-black/30 border border-gray-800 rounded p-3 h-full">
            <div className="social-private-label text-[10px] text-gray-500 uppercase tracking-widest mb-2">{label}</div>
            <div className={`social-private-value text-sm font-serif leading-relaxed ${accent}`}>{value?.trim() || "暂无记录"}</div>
        </div>
    );
                                ...
                                <div className="relative overflow-hidden rounded-[1.35rem] border border-wuxia-gold/20 bg-[radial-gradient(circle_at_top_right,rgba(212,175,55,0.14),transparent_34%),linear-gradient(115deg,rgba(13,10,8,0.98),rgba(31,22,15,0.94),rgba(13,10,8,0.98))] p-5 flex items-start justify-between shadow-[0_18px_42px_rgba(0,0,0,0.5)] group shrink-0">
                                    ...
                                    <div className="flex gap-6 relative z-10">
                                        ...
                                        >
                                            {当前详情主图 ? (
                                                <>
                                                    <img src={当前详情主图} alt={currentNPC.姓名} className={`w-full h-full object-cover object-top group-hover/portrait:scale-[1.04] transition-transform duration-500 ${当前角色已死亡 ? 'grayscale opacity-65' : ''}`} />
                                                    {当前角色已死亡 && (
                                                        <div className="absolute inset-x-0 bottom-0 bg-black/70 py-1 text-center text-xs tracking-[0.3em] text-gray-200">
                                                            已故
                                                        </div>
                                                    )}
                                                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover/portrait:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-2 backdrop-blur-[2px]">
