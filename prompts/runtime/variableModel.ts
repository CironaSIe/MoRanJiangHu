...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '9. 对话框人物未命中既有 `社交[]` 时，必须 `push 社交 = {...}` 新建完整 NPC 档案；命中半残档时，必须补齐 `性别/年龄/境界/身份/简介/是否主要角色/是否在场/记忆/天赋列表/出身背景/当前装备/背包/BUFF/DEBUFF/技艺/战斗数值/七部位状态`，不能继续写“未知/不详/剧情对话人物”。`当前装备` 只要求结构可追踪，未确认槽位写“无”；`背包` 没有明确随身物就写空数组。',
            ...
            '16. 任务完成不是一句状态文本：若正文或变量规划确认任务完成、提交或领奖，必须同步写回 `任务列表[i].当前状态=已完成`、目标完成状态、`奖励已发放/奖励发放人/奖励到账记录`，并把物品奖励 push 到 `角色.物品列表`、贡献/组织信用同时增加 `玩家门派.玩家贡献` 与 `玩家门派.累计贡献`、技艺熟练度和可分配属性点落到角色变量。',
            ...
            '17. 新增任务前必须查重：标题、发布人、发布地点、目标描述或剧情暗线高度相同的支线/组织任务只更新原任务，不要重复 push。开局和普通回合都应保留至少一条主线任务；没有主线时优先补一条可承接的主线。',
            '18. 每回合都必须刷新当前镜头快照：先确认 `环境` 的当前描写视角/当前位置，再根据本回合正文与对白只把明确在当前现场、出声、行动、被镜头点到、站在主角身边的人设为 `社交[i].是否在场 = true`；同时为这些 NPC 写入 `社交[i].当前位置` 与 `社交[i].位置路径`，位置路径格式为“大地点 > 中地点 > 小地点 > 具体地点”。其余旧在场 NPC 若本回合没有被当前现场确认，必须自然设为 `false`。远端、留守、待命、传闻、背景名单、曾经出现过的人都不是在场。',
            ...
            '23. 死亡/已故判定必须反复核对，只有同时满足三项才允许写入：目标 NPC 身份/姓名/索引一致；该 NPC `当前血量 = 0`；正文明确写出此人已经死亡、身亡、阵亡、战死、气绝、断气、毙命或已故；并且正文或变量命令给出明确死因（如被谁击杀、中毒、感染、失血、伤势过重等）。满足条件时必须同步写回 `当前血量 = 0`、`是否在场 = false`、`状态/生死状态/生命状态 = "死亡"`，并把明确死因写入 `死因/死亡原因/死亡描述/记忆/DEBUFF` 至少一处；缺少明确死因、只是昏死/濒死/重伤/险些身亡/差点死/未死/失踪/状态未知时，一律不得写“死亡/已故”。',
