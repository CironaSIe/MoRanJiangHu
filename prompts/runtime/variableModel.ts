...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '8. 每回合都要复核主角与 NPC 档案：主角必须具备 `角色.技艺`，所有 NPC 必须具备天赋列表、出身背景、当前装备、背包、BUFF、DEBUFF、技艺、七部位血量与状态；技艺至少覆盖炼器、炼丹、医术、阵法、符箓、机关、采集、鉴定等常见项。',
            '9. 若本次开局配置或当前存档题材模式为仙侠，主角与 NPC 还必须维护 `灵根/灵根资质/当前灵力/最大灵力/当前神识/最大神识/丹田状态/道基状态/心魔值/功德/业力`；术法、神通、法宝、阵法、符箓、神识探查等事件必须同步消耗或恢复灵力/神识。',
            '10. 技艺不是静态装饰：若正文或变量规划出现学习、试炼、炼制、采集、辨物、治伤、布阵、机关、符箓、丹器、找人学艺、读书学艺等事实，要按故事发展逻辑更新对应角色或 NPC 的技艺等级、熟练度与描述；初始技艺应由天赋列表、出身背景、身份职责和经历共同解释。',
            '10. 若正文或变量规划确认了新地点（世界/大洲/城镇/建筑/房间），要同步 push `世界.地图层级` 节点（名称/层级/父级ID/描述），六层结构：寰宇→大地点→中地点→小地点→区地点→子地点。旧坐标字段已废弃。',
            '11. 每回合都必须刷新当前镜头快照：先确认 `环境` 的当前描写视角/当前位置，再根据本回合正文与对白只把明确在当前现场、出声、行动、被镜头点到、站在主角身边的人设为 `社交[i].是否在场 = true`；同时为这些 NPC 写入 `社交[i].当前位置` 与 `社交[i].位置路径`，位置路径格式为“大地点 > 中地点 > 小地点 > 具体地点”。其余旧在场 NPC 若本回合没有被当前现场确认，必须自然设为 `false`。远端、留守、待命、传闻、背景名单、曾经出现过的人都不是在场。',
            '12. 写入 `社交[i]` 前必须先核对当前变量数据中该索引的 `id/姓名/身份/最近记忆` 是否就是正文或变量规划中的目标人物；若无法确认索引，不要把 A 的状态、死亡、伤势、位置或私密事实写到 B 身上，应优先补记忆或跳过该条。',
            '13. 若正文明确确认某个 NPC 死亡、身亡、阵亡、战死、气绝、断气、毙命或已故，必须同步写回该 NPC：`当前血量 = 0`、`是否在场 = false`、`状态/生死状态/生命状态 = "死亡"`，并在 `DEBUFF` 中保留“死亡”状态；“昏死、濒死、重伤、险些身亡、差点死、未死”不得按死亡处理。',
            ...(survivalNeedsEnabled
                ? [[
                    '13. 生理系统开启时，要把时间推进、休整、进食、饮水、赶路、熬战等事实对应到精力',
                    构建修炼体系附加块('、内力'),
                    '、饱腹、口渴等变量联动。'
                ].join('')]
                : [])
