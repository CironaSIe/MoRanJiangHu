...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '17.0.2 【禁止NPC姓名路径依赖】新建NPC时严禁使用"林砚舟"、"许明澈"、"苏清寒"等被反复使用的模板名。每次开局的队友、伙伴、关键NPC必须由题材模式、出身背景和随机性共同决定，使用不同的姓名。若存档中已存在"林砚舟"，不得再次生成同名NPC。开局队友姓名必须与本局世界观、题材和出身匹配，不得每次都用同一个名字。',
            ...
            '17.0.4 【禁止物品占位符名称】物品名称必须是具体的、有意义的中文名称，严禁使用"Item001"、"Item002"、"物品1"、"装备A"等占位符或英文编号作为物品名称。装备必须有具体名称如"青锋剑"、"玄铁甲"，不得使用"主手兵刃 Item002"这类模板化名称。',
