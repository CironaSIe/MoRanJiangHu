...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '9. 对话框人物或对白候选未命中既有 `社交[]` 时，只有在正文、变量规划或本回合命令已经提供了足够强的长期承接证据时，才允许 `push 社交 = {...}` 新建完整 NPC 档案；若只是一次性泛称、临时拦路、单句应答、背景陪衬、未坐实代称或纯流程性对白对象，就不要为了补对白而硬塞进长期 `社交[]`。命中半残档时，必须补齐 `性别/年龄/境界/身份/简介/是否主要角色/是否在场/记忆/天赋列表/出身背景/当前装备/背包/BUFF/DEBUFF/技艺/战斗数值/七部位状态`，不能继续写“未知/不详/剧情对话人物”。`当前装备` 只要求结构可追踪，未确认槽位写“无”；`背包` 没有明确随身物就写空数组。',
            ...
            '13. 货币语义必须跟随题材模式并保持底层统一换算：武侠用铜钱/银子/金元宝；仙侠用下品/中品/上品灵石，但允许银两、香火、凡间零钱等并行流通；灵气复苏日常用人民币/电子支付，复苏交易用灵晶、异常物资、研究额度、情报并折回复苏信用点；都市修仙日常用人民币、电子支付，修行圈高端交易才用灵石/法器/药材/情报并折回信用点；现代都市只用人民币、合同、工资、债务等现代经济口径；末日丧尸以食物、饮水、药品、弹药、电池、燃油、工具、情报和营地信用为主。若金钱以实体货币、钱袋、银票、灵石、营地配给券等实际载体直接交到主角手里，优先落为 `角色.物品列表` 中的 `货币:*` 物品；若只是电子转账、信用点记账、合同工资、系统结算或其他无实体载体余额，则优先写入 `角色.金钱`。`货币:上层货币`、`货币:中层货币`、`货币:底层货币` 用于固定结算货币，`货币:凡间`、`货币:灵石`、`货币:香火` 等可作为自由分类并行存在。',
            ...
            '16. 任务完成不是一句状态文本：若正文或变量规划确认任务完成、提交或领奖，必须同步写回 `任务列表[i].当前状态=已完成`、目标完成状态、`奖励已发放/奖励发放人/奖励到账记录`，并把物品奖励 push 到 `角色.物品列表`、贡献/组织信用同时增加 `玩家门派.玩家贡献` 与 `玩家门派.累计贡献`、技艺熟练度和可分配属性点落到角色变量。若正文确认交出、消耗或提交任务道具，必须对对应 `角色.物品列表[i]` 执行 `delete` 或 `sub 堆叠数量`，不要只改任务状态而让任务道具留在背包。',
