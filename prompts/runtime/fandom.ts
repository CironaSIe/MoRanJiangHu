...
const 默认境界映射: 境界映射项[] = [
    ...
    { level: 8, label: '聚息境四重' },
    { level: 9, label: '归元境一重' },
    { level: 10, label: '归元境二重' },
    { level: 11, label: '归元境三重' },
    { level: 12, label: '归元境四重' },
    { level: 13, label: '御劲境一重' },
    { level: 14, label: '御劲境二重' },
    { level: 15, label: '御劲境三重' },
    { level: 16, label: '御劲境四重' },
    { level: 17, label: '化罡境一重' },
    { level: 18, label: '化罡境二重' },
    { level: 19, label: '化罡境三重' },
    { level: 20, label: '化罡境四重' },
...
export const 默认累计境界分段映射提示词 = [
    ...
    '- 开脉境一重 / 二重 / 三重 / 四重：`1 / 2 / 3 / 4`',
    '- 聚息境一重 / 二重 / 三重 / 四重：`5 / 6 / 7 / 8`',
    '- 归元境一重 / 二重 / 三重 / 四重：`9 / 10 / 11 / 12`',
    '- 御劲境一重 / 二重 / 三重 / 四重：`13 / 14 / 15 / 16`',
    '- 化罡境一重 / 二重 / 三重 / 四重：`17 / 18 / 19 / 20`',
...
export const 默认累计境界差距口径提示词 = [
    ...
    '- 参考口径：',
    '  - 开脉境二重（`2`）对开脉境四重（`4`），属小差距中的上段',
    '  - 化罡境圆满（`20`）对通玄境初期（`21`），差值虽为 `1`，但已跨大境，气机质量与掌控力会出现断层感',
    '  - 神照境（`27`）对返真境（`33`），属压制差距中的高段，取胜前提需要极强外部条件'
].join('\n');

export const 默认累计境界终点文案提示词 = '- 当前境界文案位于本段终点写法：`开脉境四重 / 聚息境四重 / 归元境四重 / 御劲境四重 / 化罡境四重 / 通玄境圆满 / 神照境 / 返真境 / 天人境`';
...
export const 默认累计境界速查提示词 = '凡人/普通人/未入境=正式映射值1之前的概念层；开脉一至四重=1~4；聚息一至四重=5~8；归元一至四重=9~12；御劲一至四重=13~16；化罡一至四重=17~20；通玄初/中/圆满=21/22/24；神照/返真/天人=27/33/43。';

export const 仙侠累计境界分段映射提示词 = [
    '- 凡人/未入道：正式映射值 `1` 之前的概念层，不占正式累计境界值。',
    '- 炼气一层 / 二层 / 三层 / 四层：`1 / 2 / 3 / 4`',
    '- 炼气五层 / 六层 / 七层 / 八层：`5 / 6 / 7 / 8`',
    '- 炼气九层 / 十层 / 十一层 / 圆满：`9 / 10 / 11 / 12`',
    '- 筑基初期 / 中期 / 后期 / 圆满：`13 / 14 / 15 / 16`',
    '- 金丹初期 / 中期 / 后期 / 圆满：`17 / 18 / 19 / 20`',
    '- 元婴初期 / 中期 / 圆满：`21 / 22 / 24`',
    '- 化神期：`27`',
    '- 炼虚期：`33`',
    '- 合体期：`43`'
...
export const 仙侠累计境界能力边界提示词 = [
    '- 境界能力边界：',
    '  - 炼气：初入道途，以灵力运转、低阶术法、符箓法器和基础感知为主，仍受凡俗军阵、毒药、地势和围攻威胁',
    '  - 筑基：道基成形，术法与法器承载力明显增强，可短时御器或借阵腾挪，但仍受灵力、伤势和环境约束',
    '  - 金丹：金丹成核，攻防质量出现大境断层，能压制低阶修士，但不能无代价常态毁城、改天象或复活亡者',
    '  - 元婴：神魂与法身雏形增强，可远遁、夺势、破阵，但每次高阶神通都要消耗灵力、神识、法宝或寿元代价',
    '  - 化神/炼虚/合体：高端修士能影响区域战局与宗门格局，但仍受天道规则、因果、资源、伤势、阵法和同阶制衡'
].join('\n');

export const 仙侠累计境界文案规则提示词 = [
    '- 凡人/未入道：可直接作为叙事文案使用，但不写成正式累计境界值。',
    '- 炼气：固定采用 `炼气一层` 至 `炼气十一层`，终点为 `炼气圆满`。',
    '- 筑基/金丹：固定采用 `初期 / 中期 / 后期 / 圆满` 四段写法。',
    '- 元婴：固定采用 `元婴初期 / 元婴中期 / 元婴圆满` 三段收束写法。',
    '- 化神以上：固定采用 `化神期 / 炼虚期 / 合体期`。'
].join('\n');

export const 仙侠累计境界差距口径提示词 = [
    '- 凡人/未入道对正式入道者：若无阵法、军械、毒药、法器、地利或人数优势，默认属于明显劣势甚至断层劣势。',
    '- 累计境界差值统一按以下层次体现：',
    '  - 小差距：`1~2`，体现灵力量、术法熟练度、神识稳定性与爆发窗口优势',
    '  - 明显差距：`3~5`，低位方需要法器、阵法、克制功法或奇策来弥补',
    '  - 压制差距：`6~9`，正面硬拼通常进入稳弱势，低位方更依赖外物和环境',
    '  - 断层差距：`10+`，灵力质量、神识压迫、护体法力和术法规模会出现明显代差',
    '- 参考口径：',
    '  - 炼气三层（`3`）对炼气四层（`4`），属小差距',
    '  - 炼气圆满（`12`）对筑基初期（`13`），差值虽为 `1`，但已跨大境，道基质量会出现断层感',
    '  - 元婴圆满（`24`）对化神期（`27`），属明显跨阶压制，取胜前提需要极强外部条件'
].join('\n');

export const 仙侠累计境界终点文案提示词 = '- 当前境界文案位于本段终点写法：`炼气四层 / 炼气八层 / 炼气圆满 / 筑基圆满 / 金丹圆满 / 元婴圆满 / 化神期 / 炼虚期 / 合体期`';

export const 仙侠累计境界阶段推进提示词 = [
    '- 阶段推进统一采用以下跳转表：',
    '  - 炼气低阶：`1→2→3→4`',
    '  - 炼气中阶：`5→6→7→8`',
    '  - 炼气高阶至圆满：`9→10→11→12`',
...
export const 仙侠累计境界硬边界提示词 = [
    '- 仙侠能力必须受境界、灵力、神识、法宝、阵法、丹药、传承、伤势和因果代价约束。',
    '- 御器飞行、遁术、神通和大范围术法可以出现，但必须写出持续时间、消耗、准备、反噬或环境限制。',
    '- 禁止无代价常态复活、无视距离瞬移、无条件改天换地、低阶随手碾压高阶，跨大境胜利必须依赖明确外部条件。'
].join('\n');

export const 仙侠累计境界速查提示词 = '凡人/未入道=正式映射值1之前的概念层；炼气一至四层=1~4；炼气五至八层=5~8；炼气九/十/十一/圆满=9/10/11/12；筑基初/中/后/圆满=13/14/15/16；金丹初/中/后/圆满=17/18/19/20；元婴初/中/圆满=21/22/24；化神/炼虚/合体=27/33/43。';

export const 获取硬编码仙侠境界名称 = (levelRaw: unknown): string => {
    const numeric = Number(levelRaw);
    if (!Number.isFinite(numeric)) return '';
    const level = Math.max(1, Math.floor(numeric));
    const exact = 默认仙侠境界映射.find((item) => item.level === level);
...
const 构建自定义境界九阶命名提示词 = (schema: 境界映射母板): string => {
    const sections = [
        { levels: [1, 2, 3, 4] as const },
        { levels: [5, 6, 7, 8] as const },
        { levels: [9, 10, 11, 12] as const },
    ...
    const getLabel = (level: number): string => schema.mapping.find((item) => item.level === level)?.label || `累计值${level}`;
    const singles = [
        { level: 27 }, { level: 33 }, { level: 43 }
    ];
    const stageLines = sections.map((section) => {
        const labels = section.levels.map((level) => getLabel(level));
        const groupName = labels[0]?.replace(/[一二三四1-4][重层阶]?$/, '').trim() || `阶段${section.levels[0]}`;
        const values = section.levels.map((level) => String(level)).join(' / ');
        return `  - ${groupName}（\`${values}\`）`;
    });
    const singleLines = singles.map((item) => {
        const label = getLabel(item.level);
...
const 构建自定义境界能力边界提示词 = (schema: 境界映射母板): string => {
    const getLabel = (level: number): string => schema.mapping.find((item) => item.level === level)?.label || `累计值${level}`;
    const allLabels = schema.mapping.map((item) => item.label);
    const isModernLike = allLabels.some((label) => /普通人|熟练者|专家|骨干|精英|王牌|幸存者|适应者|营地|战术|见习|初阶|中阶|高阶|大师|英雄|新人|资深|轮回者|基因锁|超凡/i.test(label));
    if (isModernLike) {
        return [
            '- 境界能力边界：',
            `  - ${getLabel(1)}~${getLabel(4)}：基础水平，受现实约束和常规风险限制`,
            `  - ${getLabel(5)}~${getLabel(8)}：进阶水平，具备一定专业能力但仍受环境和资源限制`,
            `  - ${getLabel(9)}~${getLabel(12)}：中坚水平，可承担关键任务但仍非不可战胜`,
            `  - ${getLabel(13)}~${getLabel(16)}：高阶水平，在局部领域具备显著优势但仍受制于资源消耗和外部条件`,
            `  - ${getLabel(17)}~${getLabel(20)}：精英水平，可独当一面但仍会受伤和消耗`,
            `  - ${getLabel(21)}~${getLabel(24)}：王牌/英雄级，短时强大但仍有极限和代价`,
            `  - ${getLabel(27)}~${getLabel(43)}：顶层水平，能左右高端局势，但并非不可匹敌`
        ].join('\n');
    }
    return [
        '- 境界能力边界：',
        `  - ${getLabel(1)}~${getLabel(4)}：强于常人，但仍明显受围攻、地形和资源压制`,
        `  - ${getLabel(5)}~${getLabel(8)}：成熟好手，仍非刀枪不入`,
        `  - ${getLabel(9)}~${getLabel(12)}：地方强者门槛，受毒药、机关和疲劳约束`,
        `  - ${getLabel(13)}~${getLabel(16)}：攻防转换成熟，可成大势力中坚，但依旧受伤势和围攻约束`,
        `  - ${getLabel(17)}~${getLabel(20)}：成熟高手，短时高机动与压制感增强，但仍会疲和伤`,
        `  - ${getLabel(21)}~${getLabel(24)}：一流上位，能左右高端战局，但仍受环境和资源约束`,
        `  - ${getLabel(27)}~${getLabel(43)}：顶尖/顶峰，能力极强但仍不是仙神，不能常态飞天、改天换地`
        ].join('\n');
};

const 构建自定义境界文案规则提示词 = (schema: 境界映射母板): string => {
    const getLabel = (level: number): string => schema.mapping.find((item) => item.level === level)?.label || `累计值${level}`;
    const allLabels = schema.mapping.map((item) => item.label);
    const usesTierSystem = allLabels.some((label) => /一重|二重|三重|四重|一阶|二阶|三阶|四阶/i.test(label));
    if (usesTierSystem) {
        const stageEndLabels = [4, 8, 12, 16, 20].map((level) => getLabel(level)).join(' / ');
        return [
            `- 凡人/普通人/未入境：可直接作为叙事文案使用，但不写成正式累计境界值。`,
            `- ${getLabel(1)}至${getLabel(20)}：默认采用对应境界标签写法。`,
            `- ${getLabel(21)}~${getLabel(24)}：采用分段收束写法。`,
            `- ${getLabel(27)}及以上：${[27, 33, 43].map((level) => getLabel(level)).filter(Boolean).join(' / ')}`
        ].join('\n');
    }
    return `- \`角色.境界\`、\`社交[i].境界\`、\`战斗.敌方[i].境界\` 等正式文案统一直接取自当前存档【境界映射母板】对应值。
- 同段推进、破境、校准与展示时，优先参照【境界映射母板】、【终点文案】、【阶段推进表】与【大境突破表】，不要回写默认武侠术语。`;
};

const 构建自定义境界终点文案提示词 = (schema: 境界映射母板): string => {
    const getLabel = (level: number): string => schema.mapping.find((item) => item.level === level)?.label || `累计值${level}`;
    return `- 当前境界文案位于本段终点写法：\`${[4, 8, 12, 16, 20, 24, 27, 33, 43].map((level) => getLabel(level)).filter(Boolean).join(' / ')}\``;
};

const 构建默认境界区块集合 = (schema: 境界映射母板): 境界区块集合 => {
    const isCustomMapping = schema.source !== 'default' || schema.mapping !== 默认境界映射;
    if (isCustomMapping) {
        const customNineNamePrompt = 构建自定义境界九阶命名提示词(schema);
        const customAbilityPrompt = 构建自定义境界能力边界提示词(schema);
        const customRulePrompt = 构建自定义境界文案规则提示词(schema);
        const customEndpointPrompt = 构建自定义境界终点文案提示词(schema);
        return {
            题材模式: '武侠',
            境界映射母板: schema.mapping.map((item) => `${item.level} => ${item.label}`).join('\n'),
            九阶命名与能力边界: [customNineNamePrompt, customAbilityPrompt].join('\n'),
            九阶命名提示词: customNineNamePrompt,
            能力边界提示词: customAbilityPrompt,
            境界差距口径: 默认累计境界差距口径提示词,
            终点文案: customEndpointPrompt,
            阶段推进表: 默认累计境界阶段推进提示词,
            大境突破表: 默认累计境界大境突破提示词,
            武侠硬边界: 默认累计境界武侠硬边界提示词,
            分段映射提示词: 构建境界分段映射提示词(schema),
            文案规则提示词: customRulePrompt
        };
    }
...
const 构建仙侠境界区块集合 = (): 境界区块集合 => {
    const schema = 构建仙侠境界母板();
    const 九阶命名与能力边界 = [仙侠累计境界九阶命名提示词, 仙侠累计境界能力边界提示词].join('\n');
    return {
        题材模式: '仙侠',
        境界映射母板: schema.mapping.map((item) => `${item.level} => ${item.label}`).join('\n'),
        九阶命名与能力边界,
        九阶命名提示词: 仙侠累计境界九阶命名提示词,
        能力边界提示词: 仙侠累计境界能力边界提示词,
        境界差距口径: 仙侠累计境界差距口径提示词,
        终点文案: 仙侠累计境界终点文案提示词,
        阶段推进表: 仙侠累计境界阶段推进提示词,
        大境突破表: 仙侠累计境界大境突破提示词,
        武侠硬边界: 仙侠累计境界硬边界提示词,
        分段映射提示词: 仙侠累计境界分段映射提示词,
        文案规则提示词: 仙侠累计境界文案规则提示词
    };
};
...
export const 构建同人运行时提示词包 = (params: {
    ...
    const 同人设定摘要 = [
        ...
        `- ${保留原著角色文本}`,
        isXianxia
            ? '- 当前为仙侠题材：境界体系使用项目内置固定映射，不读取 AI 生成的武侠/同人境界名称。'
...
export const 应用境界体系区块替换 = (
    sourceText: string,
    bundle: Pick<同人运行时提示词包, '境界区块集合'> & Partial<Pick<同人运行时提示词包, 'enabled'>>
): string => {
    const source = typeof sourceText === 'string' ? sourceText : '';
    if (!source || !bundle.境界区块集合) return source;
    ...
    const replaced = replacements.reduce((text, [target, replacement]) => (
        target && replacement ? text.split(target).join(replacement) : text
    ), source);

    if (bundle.境界区块集合.题材模式 !== '仙侠') return replaced;
    return replaced
        .replace(
            '【武侠修炼体系｜累计境界、修炼节奏、资源代价与突破门槛】',
            '【仙侠修炼体系｜累计境界、修炼节奏、资源代价与突破门槛】'
        )
        .replace(
            '修炼主轴是“内息、招式、心境”三轴，不进入仙侠常态飞行、起死回生、法术轰击叙事。',
            '修炼主轴是“灵力、术法/法宝、神识心境”三轴；允许御器飞行、遁术、阵法、丹药与神通表现，但必须受境界、资源、传承、伤势与环境条件约束。'
        )
        .replace('  - 开脉：文案从“X境N重”改为“X境N+1重”', '  - 炼气：文案从“炼气N层”改为下一层；炼气十一层后推进为“炼气圆满”')
        .replace('  - 聚息到化罡：文案从“X境N重”改为“X境N+1重”', '  - 筑基到金丹：文案按“初期→中期→后期→圆满”推进；元婴按“初期→中期→圆满”推进')
        .replace('## 9. 武侠硬边界与落地纪律', '## 9. 仙侠硬边界与落地纪律');
};
