...
export const 构建地图更新用户提示词 = (params: {
    ...
    if (params.mode === 'memory_regenerate') {
        ...
        return [
            ...
            '6. 同一个角色只能有一个最细叶子位置；不要把同一人同时写在寰宇/大地点/中地点/小地点/区地点/子地点多个层级。父级视图由系统通过层级链覆盖显示。',
            '7. 地点若能判断势力控制或势力影响，必须补充 控制势力 / 势力影响 / 势力标签；看不出势力时不要硬编。',
            '8. 每个地点节点如果能判断有谁在场，必须补充 在场人物 字段（字符串数组，填写在场角色姓名）。根据正文和人物位置线索判断每个角色所在的最细叶子地点，只在最细叶子节点填写在场人物，上级节点由系统自动汇总。',
