...
const 创建默认门派任务列表 = (sectName: string, seed = 0): 详细门派结构['任务列表'] => {
    const location = 按种子取项(['山门外市集', '旧驿道', '外务堂', '藏经阁前院', '灵田边', '后山栈道'], seed, 3);
    const trouble = 按种子取项(['商队纠纷', '药材短缺', '散修试探', '旧账未清', '阵纹失修', '弟子争执'], seed, 7);
    return [
    {
        id: 'sect_default_patrol',
        标题: `${location}巡查`,
        描述: `${sectName}近日受${trouble}牵动，外务堂需要弟子去${location}查明缘由，并把结果回报给执事。`,
        类型: '日常',
        难度: '1星',
        发布日期: '1:01:01:00:00',
        截止日期: '1:01:03:23:59',
    ...
    },
    {
        ...
        标题: `${trouble}委托`,
        描述: `${sectName}内部正在处理${trouble}，此事牵连主角当前处境与门中人情，适合接下后顺势追查。`,
        类型: '建设',
        难度: '1星',
        发布日期: '1:01:01:00:00',
        截止日期: '1:01:05:23:59',
    ...
    },
    {
        ...
        标题: '门中历练',
        描述: `${sectName}安排年轻弟子结伴历练，目标会随附近局势变化，可从同门名录中挑选同行者。`,
        类型: '历练',
        难度: '2星',
        发布日期: '1:01:01:00:00',
        截止日期: '1:01:10:23:59',
...
const 从门派任务创建通用任务列表 = (sectName: string, missions: 详细门派结构['任务列表']): 任务结构[] => (
    Array.isArray(missions) ? missions.map((mission) => ({
        标题: 取文本(mission?.标题, '门派差遣'),
        描述: 取文本(mission?.描述, `${sectName}交付的一桩门中事务。`),
        类型: '门派',
        发布人: sectName,
        发布地点: sectName,
        推荐境界: 取文本(mission?.难度, '按门派差遣'),
        截止时间: 取文本(mission?.截止日期) || undefined,
        当前状态: 门派任务状态转任务状态(取文本(mission?.当前状态)),
        目标列表: [{
            描述: 取文本(mission?.描述, mission?.标题 || '处理门派事务'),
            当前进度: mission?.当前状态 === '已完成' ? 1 : 0,
            总需进度: 1,
            完成状态: mission?.当前状态 === '已完成'
        }],
        奖励描述: [
            mission?.奖励贡献 ? `门派贡献 +${mission.奖励贡献}` : '',
            mission?.奖励资金 ? `铜钱 +${mission.奖励资金}` : '',
            ...(Array.isArray(mission?.奖励物品) ? mission.奖励物品 : [])
        ].filter(Boolean),
        剧情暗线: `门派任务：${sectName}的「${取文本(mission?.标题, '门派差遣')}」必须结合当前剧情、地点、在场人物与门派近况推进。`
    })) : []
);
...
const 补齐门派重要成员 = (sourceMembers: unknown): 详细门派结构['重要成员'] => {
    ...
    return 重命名重复女性NPC列表(members) as 详细门派结构['重要成员'];
};
...
const 生成开局门派名称 = (charData: 角色数据结构, openingConfig?: OpeningConfig): string => {
    ...
    const prefix = openingConfig?.题材模式 === '仙侠'
        ? ['青崖', '玄衡', '云岫', '烬霞', '澄霄', '归藏', '天炉', '寒照']
        : ['折柳', '玄墨', '听潮', '照影', '归雁', '松风', '问剑', '长鲸'];
    const suffix = openingConfig?.题材模式 === '仙侠'
        ? ['仙宗', '剑宗', '道院', '灵门', '玄府', '真宫']
        : ['山庄', '门', '帮', '武馆', '剑派', '堂'];
...
const 创建默认同门名录 = (sectName: string, openingConfig?: OpeningConfig): 详细门派结构['重要成员'] => {
    ...
    const roles = ['掌事师叔', '外务执事', '内门师兄', '内门师姐', '外门弟子', '外门弟子', '杂役弟子', '藏经阁值守'];
    ...
    return Array.from({ length: count }, (_, index) => {
        ...
        return {
            ...
            境界: openingConfig?.题材模式 === '仙侠'
                ? 按种子取项(['炼气中期', '炼气后期', '筑基初期', '炼气初期'], seed, index)
                : 按种子取项(['锻体境', '开脉境', '凝气境', '内息小成'], seed, index),
            身份: identity,
            简介: `${sectName}${identity}，负责${按种子取项(['外务传令', '照看新弟子', '巡守山门', '整理典籍', '采办物资', '维持门规'], seed, index)}。`
        };
    });
};
...
const 创建开局门派种子数据 = (
    ...
    return {
        ...
        material,
        build,
        intro: `${sectName}${按种子取项(['立于山城要道', '据守旧岭山门', '传承数代', '近年声势渐起'], seed, 5)}，宗旨是${按种子取项(ideals, seed, 9)}。`,
        rules: [
            '不可同门相残',
...
const 推导门派规模数据 = (source: any, displayName: string) => {
    ...
    return {
        ...
        月俸规则: {
            ...
            发放说明: 取文本(stipend?.发放说明, `${displayName}按月根据职位、累计贡献、贡献等级、门派规模与财富状况发放俸禄。`)
        },
...
export const 创建开场基础状态 = (charData: 角色数据结构, _worldConfig: WorldGenConfig, openingConfig?: OpeningConfig) => {
    ...
    const 门派任务 = 是否无门派标识(玩家门派.ID)
        ...
        : 从门派任务创建通用任务列表(
            玩家门派.名称,
            创建默认门派任务列表(玩家门派.名称, 生成稳定哈希(`${玩家门派.ID}|opening_tasks`))
        );
