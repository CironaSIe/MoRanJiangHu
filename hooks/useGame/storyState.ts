...
const 补全门派职位 = (source: any, totalContribution = 0, fallback = '无'): string => {
    ...
    const sortedRanks = Object.entries(门派职位贡献门槛).sort(([, a], [, b]) => a - b);
    sortedRanks.forEach(([rank, required]) => {
...
const 按性别比例取性别 = (ratio: string | { 男: number; 女: number; 男娘: number; 扶她: number } | undefined, seed: number, index: number): string => {
    if (ratio && typeof ratio === 'object') {
        ...
        if (total <= 0) return '男';
        const entries = [
            { type: '男' as const, weight: ratio.男 },
            { type: '女' as const, weight: ratio.女 },
            { type: '男娘' as const, weight: ratio.男娘 },
            { type: '扶她' as const, weight: ratio.扶她 },
        ].filter(e => e.weight > 0);
        const deterministicRand = ((seed * 9301 + index * 49297) % 233280) / 233280;
        let cumulative = 0;
        for (const entry of entries) {
            cumulative += entry.weight / total;
            if (deterministicRand <= cumulative) return entry.type;
        }
