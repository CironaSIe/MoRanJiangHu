...
const 创建默认同门列表 = (sectName: string): 详细门派结构['重要成员'] => ([
    { id: 'sect_member_master', 姓名: '沈若嫣', 性别: '女', 年龄: 19, 境界: '开脉境二重', 身份: '少庄主', 简介: `主理${sectName}内外事务，行事果断。` },
    { id: 'sect_member_elder', 姓名: '杨震', 性别: '男', 年龄: 42, 境界: '通玄境圆满', 身份: '执事', 简介: '负责传功与巡山，赏罚分明。' },
    { id: 'sect_member_true', 姓名: '陆明澈', 性别: '男', 年龄: 24, 境界: '开脉境九重', 身份: '真传弟子', 简介: '常随长老外出办事，是同辈里最早独当一面的弟子。' },
    { id: 'sect_member_inner', 姓名: '苏晚晴', 性别: '女', 年龄: 21, 境界: '开脉境五重', 身份: '内门弟子', 简介: '负责整理藏经阁借阅名册，待人温和但记性极好。' },
    { id: 'sect_member_apprentice', 姓名: '曹雄', 性别: '男', 年龄: 17, 境界: '锻劲境初期', 身份: '外门弟子', 简介: '新入门不久，常在练武场切磋。' },
    { id: 'sect_member_outer_lan', 姓名: '许青萝', 性别: '女', 年龄: 16, 境界: '锻劲境圆满', 身份: '外门弟子', 简介: '消息灵通，常帮同门打听任务布告的风向。' },
    { id: 'sect_member_worker', 姓名: '赵平安', 性别: '男', 年龄: 15, 境界: '未入境', 身份: '杂役弟子', 简介: '管着偏院柴米和药圃杂务，很多门内琐事都绕不开他。' },
    { id: 'sect_member_minor_01', 姓名: '韩小霜', 性别: '女', 年龄: 18, 境界: '锻劲境中期', 身份: '外门弟子', 简介: '普通同门，常在晨课与巡山队伍中露面。' }
]);
...
export const 规范化门派状态 = (raw?: any): 详细门派结构 => {
    ...
    return {
        ...
        重要成员: Array.isArray(source?.重要成员) && source.重要成员.length > 0 ? source.重要成员 : (isActiveSect ? 创建默认同门列表(displayName) : [])
    };
};
...
export const 创建开场基础状态 = (charData: 角色数据结构, _worldConfig: WorldGenConfig, openingConfig?: OpeningConfig) => ({
    ...
    世界: 创建开场空白世界(),
    战斗: 创建开场空白战斗(),
    玩家门派: 创建开局门派状态(charData, openingConfig),
    任务列表: [],
    约定列表: [],
    剧情: 创建开场空白剧情(),
    剧情规划: 创建空剧情规划(),
