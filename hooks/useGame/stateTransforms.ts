...
const 标准化出身背景 = (raw: any, fallback = 默认背景模板): { 名称: string; 描述: string; 效果: string; 初始物品?: { 名称: string; 数量?: number; 描述?: string; }[] } => {
    ...
    const 初始物品 = rawItems
        ...
        .filter(Boolean) as { 名称: string; 数量?: number; 描述?: string; }[] | undefined;
    return {
...
const 规范化角色物品容器映射 = (rawRole?: any, options?: 玩家BUFF规范化选项): 角色数据结构 => {
    ...
    // 若出身背景定义了初始物品列表，使用背景物并跳过系统自动预设
    const 背景初始物品 = (role as any)?.出身背景?.初始物品;
    if (Array.isArray(背景初始物品)) {
        const 背景物品 = 背景初始物品
            .map((entry: any) => {
