...
import { 古风丹药预设名称集合, 补齐自动丹药预设, 生存补给预设名称集合 } from '../../utils/autoConsumables';
...
const 古风丹药题材禁用正则 = /辟谷丹|回气丹|凝元丹|破境丹|筑基丹|引气丹|聚灵丹|混元丹|丹药|修炼丹|突破丹|灵丹|仙丹|药丸|丹丸/u;
const 题材现代化时应移除古风丹药 = (item: any): boolean => {
    ...
    if (/auto_pill_|pill|cultivation pill|breakthrough pill|qi recovery pill/i.test(text)) return true;
    return 古风丹药题材禁用正则.test(text);
};
...
const 角色疑似现代或末日题材 = (role: any): boolean => {
    const text = [
        ...
        role?.背景,
        role?.当前位置,
        ...(Array.isArray(role?.物品列表) ? role.物品列表 : []).flatMap((item: any) => [
            item?.ID,
            item?.名称,
            item?.类型,
            item?.描述,
    ...
    return /(末日|丧尸|废土|营地|幸存|感染|哨所|哨塔|防毒|滤芯|军用|迷彩|战术|军刺|军靴|军服|压缩饼干|信用点|燃油|电池|弹药|急救|绷带)/u.test(text);
};
...
const 规范化角色物品容器映射 = (rawRole?: any, options?: 玩家BUFF规范化选项): 角色数据结构 => {
    ...
    role.物品列表 = deduped.filter((item: any) => {
        ...
        if (shouldStripCultivationPills && (古风丹药预设名称集合.has(name) || 题材现代化时应移除古风丹药(item))) {
            return false;
        }
        return true;
    });
    ...
    if (!是否已补过) {
        role.物品列表 = 补齐自动丹药预设(role.物品列表, {
            ...
            题材模式: shouldStripCultivationPills && topicProfile.group !== 'apocalypse' ? '末日丧尸' : options?.题材模式
        });
