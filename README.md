...
```bash
npm run cnb:workspace -- --repo bacon159-2026/comfyui-ql --branch main
```
...
```bash
node scripts/cnb-image-backend-sync.mjs --workspace --repo bacon159-2026/comfyui-ql --branch main
```
...
CNB_REPO=bacon159-2026/comfyui-ql
CNB_BRANCH=main
CNB_REF=
...
- Android 自定义 scheme 的 `oauth` 是 host，实际 path 是 `/github/callback`；Manifest 里不要把自定义 scheme 的 path 写成 `/oauth/github/callback`。
